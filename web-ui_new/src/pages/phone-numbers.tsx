...
export default function PhoneNumbers() {
  ...
  const searchAvailableNumbers = async () => {
    setIsSearching(true);
    try {
      const numbers = await phoneNumbersService.searchAvailable(searchCriteria);
  ...
  const handlePurchaseNumber = async (number: AvailablePhoneNumber) => {
    ...
    try {
      await phoneNumbersService.purchase({ phoneNumber: number.phoneNumber });
  ...
  const handleDeleteNumber = async (numberId: string, phoneNumber: string) => {
    ...
    try {
      await phoneNumbersService.delete(numberId, true);
  ...
  const filteredNumbers = phoneNumbers?.filter((number) => {
    ...
    const matchesStatus = statusFilter === "ALL" ||
                          (statusFilter === "AVAILABLE" && !number.campaignId) ||
                          (statusFilter === "ASSIGNED" && number.campaignId);
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Phone Number Management" />
      </Head>
                      ...
                      <TableRow key={number.id}>
                        ...
                        <TableCell>
                          <StatusBadge
                            ...
                            type={number.isActive ? 'success' : 'default'}
                          />
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2 text-xs">
                            <Badge variant="outline" className="text-xs">{number.type}</Badge>
                            <Badge variant="outline" className="text-xs">{number.provider}</Badge>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-1 text-sm text-gray-600">
                            <MapPin className="h-3 w-3" />
                            {(number as any).tenant?.name || 'N/A'}
                          </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          {(number as any).platformTrunkId ? (
                            <div className="flex items-center gap-1">
                              <div className="h-2 w-2 bg-green-500 rounded-full animate-pulse" />
                              <span className="text-xs text-gray-600">Platform Trunk</span>
                            </div>
                          ) : (number as any).livekitTrunkId ? (
                            <div className="flex items-center gap-1">
                              <div className="h-2 w-2 bg-blue-500 rounded-full" />
                              <span className="text-xs text-gray-600">LiveKit Trunk</span>
                            </div>
                          ) : (
