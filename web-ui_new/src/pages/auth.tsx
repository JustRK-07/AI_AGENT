...
export default function Auth() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If super admin, show tenant selector modal
      if (response.user.isAdmin) {
        setIsLoading(false);
        setShowTenantSelector(true);
      } else {
        // Regular user - redirect to dashboard immediately
        await router.push("/");
      }
    } catch (error: any) {
  ...
  return (
    <>
                  ...
                  <div className="text-center pt-3 border-t border-slate-200 dark:border-slate-700/50">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        <span className="font-semibold text-blue-600 dark:text-blue-400">Super Admin:</span> admin@system.com | admin123
                      </p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        <span className="font-semibold text-green-600 dark:text-green-400">Tenant User:</span> qateam@ytel.com | dhuxHJrz5D4nNnFmE468EzhN3yUKGL7euiDH9UCK
                      </p>
                    </div>
                  </div>
                </div>
      ...
      {/* Tenant Selector Modal - shown to super admins after login */}
      <TenantSelectorModal
