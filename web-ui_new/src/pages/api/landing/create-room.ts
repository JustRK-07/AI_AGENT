import type { NextApiRequest, NextApiResponse } from 'next';
import { AccessToken, RoomServiceClient } from 'livekit-server-sdk';

type ResponseData = {
  success: boolean;
  data?: {
    roomName: string;
    token: string;
    participantName: string;
    livekitUrl: string;
    demoType: string;
    expiresIn: number;
  };
  error?: string;
};

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<ResponseData>
) {
  if (req.method !== 'POST') {
    return res.status(405).json({
      success: false,
      error: 'Method not allowed',
    });
  }

  try {
    const { demoType } = req.body;

    if (!demoType) {
      return res.status(400).json({
        success: false,
        error: 'Demo type is required',
      });
    }

    // Fetch demo agent from backend API
    const backendUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000/api/v1';
    const agentsResponse = await fetch(`${backendUrl}/demo-agents`);

    if (!agentsResponse.ok) {
      throw new Error('Failed to fetch demo agents from backend');
    }

    const agentsData = await agentsResponse.json();
    const demoAgent = agentsData.data.find((agent: any) => agent.slug === demoType && agent.isActive);

    if (!demoAgent) {
      return res.status(404).json({
        success: false,
        error: `Demo agent "${demoType}" not found or not active`,
      });
    }

    // Get LiveKit credentials from environment
    const livekitUrl = process.env.LIVEKIT_URL || process.env.LIVEKIT_SERVER_URL;
    const apiKey = process.env.LIVEKIT_API_KEY;
    const apiSecret = process.env.LIVEKIT_API_SECRET;

    if (!apiKey || !apiSecret || !livekitUrl) {
      return res.status(500).json({
        success: false,
        error: 'LiveKit credentials not configured',
      });
    }

    // Create unique room name
    const timestamp = Date.now();
    const roomName = `demo-${demoType}-${timestamp}`;
    const participantName = `user-${timestamp}`;

    // Create room using RoomServiceClient
    const roomClient = new RoomServiceClient(livekitUrl, apiKey, apiSecret);

    // Use specialized demo agent
    const agentName = 'demo-agents';

    try {
      // Create the room with agent configuration from database
      await roomClient.createRoom({
        name: roomName,
        emptyTimeout: 900, // 15 minutes
        maxParticipants: 2, // User + AI agent
        // THIS IS KEY: Tell LiveKit to dispatch the agent to this room
        agents: [
          {
            agentName: agentName,
            metadata: JSON.stringify({
              demoType: demoType,
              purpose: 'landing-page-demo',
              // Pass full agent configuration from database
              agentConfig: {
                title: demoAgent.title,
                name: demoAgent.name,
                systemPrompt: demoAgent.systemPrompt,
                personality: demoAgent.personality,
                llmProvider: demoAgent.llmProvider || 'openai',
                llmModel: demoAgent.llmModel || 'gpt-4o-mini',
                voiceId: demoAgent.voiceId || 'default',
              },
            }),
          },
        ],
      });
      console.log(`✓ Created demo room: ${roomName} with agent: ${agentName} (${demoType})`);
      console.log(`✓ Agent config: ${demoAgent.name} - ${demoAgent.llmProvider}/${demoAgent.llmModel}`);
    } catch (roomError: any) {
      // Room might already exist, which is fine
      console.log(`Room already exists or error creating: ${roomError.message}`);
    }

    // Generate participant token
    const token = new AccessToken(apiKey, apiSecret, {
      identity: participantName,
      name: participantName,
    });

    // Grant permissions
    token.addGrant({
      roomJoin: true,
      room: roomName,
      canPublish: true,
      canPublishData: true,
      canSubscribe: true,
    });

    const jwt = await token.toJwt();

    console.log(`✓ Generated token for ${participantName} in room ${roomName}`);

    // Return room details and token
    return res.status(200).json({
      success: true,
      data: {
        roomName,
        token: jwt,
        participantName,
        livekitUrl: livekitUrl.replace('wss://', 'https://').replace('ws://', 'http://'),
        demoType,
        expiresIn: 900, // 15 minutes
      },
    });
  } catch (error: any) {
    console.error('Error creating demo room:', error);
    return res.status(500).json({
      success: false,
      error: error.message || 'Failed to create demo room',
    });
  }
}
