...
import { campaignsService, Campaign, CampaignStats } from "@/services/campaignsService";
import type { PhoneNumber } from "@/services/phoneNumbersService";
...
export default function CampaignDetail() {
  ...
  // Test call states (Note: Test call functionality requires backend implementation)
  ...
  const fetchCampaignDetails = async (campaignId: string) => {
    try {
      ...
      const [campaignData, statsData] = await Promise.all([
        ...
        campaignsService.getStats(campaignId).catch(() => null), // Stats are optional
      ]);
  ...
  const handleTestCall = async () => {
    ...
    try {
      const response = await campaignsService.initiateTestCall(campaign.id, testPhoneNumber);
      setCurrentCall(response);
      setShowTestDialog(false);
      setTestPhoneNumber('');
      toast.success(`Test call initiated successfully! Call SID: ${response.sipCallId}`);
    } catch (error: any) {
  ...
  const handleEndCall = async () => {
    ...
    // Note: End call functionality requires backend API implementation
    toast.info('End call feature coming soon! Backend API endpoint needed.');

    // TODO: Implement when backend API is ready
    // try {
    //   await campaignsService.endTestCall(campaign!.id, currentCall.callSid);
  ...
  const campaignAgents = campaign.campaignAgents || [];
  const livekitTrunk = campaign.livekitTrunk;
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              disabled={!campaign?.phoneNumbers?.length || !campaign?.campaignAgents?.length}
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-lg font-semibold">{campaign.campaignType || 'Not Set'}</span>
                  </div>
                </div>
                <Badge variant={campaign.status === 'active' ? "default" : "secondary"}>
                  {campaign.status === 'active' ? "Active" : campaign.status === 'paused' ? "Paused" : "Inactive"}
                </Badge>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-lg font-semibold">{campaignAgents.length}</span>
                  </div>
                </div>
              </div>
            </CardContent>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-lg font-semibold capitalize">{campaign.status || 'draft'}</span>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="border-0 shadow-sm">
              ...
              <CardContent className="space-y-4">
                ...
                {livekitTrunk && (
                  <div>
                    <Label className="text-sm font-medium text-gray-600">LiveKit Trunk</Label>
                    <div className="mt-1">
                      <p className="text-base font-medium">{livekitTrunk.name}</p>
                      <p className="text-sm text-gray-500">Type: {livekitTrunk.trunkType} | Status: {livekitTrunk.status}</p>
                    </div>
                  </div>
                )}
              </CardContent>
                    ...
                    <TableBody>
                      {phoneNumbers.map((number) => (
                        <TableRow key={number.id}>
                          ...
                          <TableCell>{number.type || 'N/A'}</TableCell>
                          <TableCell>
                            {number.isActive ? (
            ...
            <Card className="border-0 shadow-sm">
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="secondary">{campaignAgents.length} Total</Badge>
                </div>
              </CardHeader>
              <CardContent>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>LiveKit Agent</TableHead>
                        <TableHead>Status</TableHead>
                    ...
                    <TableBody>
                      {campaignAgents.map((campaignAgent) => (
                        <TableRow key={campaignAgent.id}>
                          ...
                          <TableCell>
                            <Badge variant="outline">{campaignAgent.agent.livekitAgentName || 'N/A'}</Badge>
                          </TableCell>
                          <TableCell>
                            {campaignAgent.agent.isActive ? (
                              <Badge variant="default" className="gap-1">
                                <CheckCircle className="h-3 w-3" />
                                Active
                              </Badge>
            ...
            <Card className="border-0 shadow-sm">
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                  ...
                  <span className="text-lg font-bold text-blue-600">{stats?.totalLeads || 0}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                  ...
                  <span className="text-lg font-bold text-green-600">{stats?.completedLeads || 0}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                  ...
                  <span className="text-lg font-bold text-red-600">{stats?.failedLeads || 0}</span>
                </div>
                    ...
                    <div className="text-sm text-blue-700 space-y-1">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-medium">
                          {campaignAgents[0]?.agent?.name || 'N/A'}
                        </span>
                      </div>
                    </div>
                  </div>
